...
/// use std::path::Path;
///
/// use singularity_code_analysis::read_file;
///
...
/// use std::path::Path;
///
/// use singularity_code_analysis::read_file_with_eol;
///
/// let path = Path::new("Cargo.toml");
...
///
/// use singularity_code_analysis::guess_language;
///
...
pub(crate) fn guess_file<S: ::std::hash::BuildHasher>(
    ...
    let include_path = if let Some(end) = include_path.strip_prefix("singularity/") {
        end
    } else {
        include_path
    };
